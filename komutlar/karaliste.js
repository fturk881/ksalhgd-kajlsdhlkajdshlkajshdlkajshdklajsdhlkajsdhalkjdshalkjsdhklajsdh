...
if (Dil == 'en') {
client.users.cache.get(Mal.id).send(`You Are Moved To White List By \`${message.author.tag}\`! Now You Can Use Commands! https://discord.gg/F9U5MUCEH5`).catch(Error => message.author.send(`\`${Mal.tag}\` Kullanıcısı Beyaz Listeye Aldın Ancak Özel Mesaj Kutusu Kapalı Olduğu İçin Mesaj Atamadım!`))
} else {
client.users.cache.get(Mal.id).send(`\`${message.author.tag}\` Tarafından Beyaz Listeye Alındın! Artık Komutları Kullanabilirsin! https://discord.gg/F9U5MUCEH5`).catch(Error => message.author.send(`\`${Mal.tag}\` Kullanıcısı Beyaz Listeye Aldın Ancak Özel Mesaj Kutusu Kapalı Olduğu İçin Mesaj Atamadım!`))
}
} else {
...
if (Dil == 'en') {
client.users.cache.get(Mal.id).send(`You Are Moved To Black List By \`${message.author.tag}\`! Reason: \`${KSebep}\`. If You Think we're Wrong Come and Speak with Us! https://discord.gg/F9U5MUCEH5`).catch(Error => message.author.send(`\`${Mal.tag}\` Kullanıcısı Kara Listeye Aldın Ancak Özel Mesaj Kutusu Kapalı Olduğu İçin Mesaj Atamadım!`))
} else {
client.users.cache.get(Mal.id).send(`\`${message.author.tag}\` Tarafından \`${KSebep}\` Sebebiyle Kara Listeye Alındın! Eğer Bir Hatamız Var İse https://discord.gg/F9U5MUCEH5`).catch(Error => message.author.send(`\`${Mal.tag}\` Kullanıcısı Kara Listeye Aldın Ancak Özel Mesaj Kutusu Kapalı Olduğu İçin Mesaj Atamadım!`))
}
}
}
